...
class Rect{
	...
	private:
		...
		*/
		...
		// Class Atributes
		GLfloat PositionX {ORIGIN_X};	// Position of the Rectangle
	...
	public:
		...
		// Constructors of the Class
			...
			Rect(float w, float h, float X, float Y, float Z): width{w}, height{h}, PositionX{X}, PositionY{Y}, PositionZ{Z}{

			}
			Rect(float w, float h, float X, float Y, float Z, GLfloat Red, GLfloat Green, GLfloat Blue) :
				width{w}, height{h}, PositionX{X}, PositionY{Y}, PositionZ{Z}, Color{Red, Green, Blue}{

			}
			Rect(float w, float h, float X, float Y, float Z, Anchor centeredOn, GLfloat Red, GLfloat Green, GLfloat Blue) :
				width{w}, height{h}, PositionX{X}, PositionY{Y}, PositionZ{Z}, anchorPlace{centeredOn}, Color{Red, Green, Blue}{
...
public:
	HLine(float size, float X1, float Y1, GLfloat Red, GLfloat Green, GLfloat Blue, bool isDotted = false, int nDots = 40)
		: Rect(size, this->linewidth, X1, Y1, 0, Rect::left, Red, Green, Blue), dotted(isDotted), numDots(nDots)
	{
		if (isDotted)
		{
			if (numDots <= 0) {
            ...
            for (int i = 0; i < numDots; i++){
                ...
                Rect* dot = new Rect(dotWidth, this->linewidth, dotX, Y1, 0, Rect::left, Red, Green, Blue);
