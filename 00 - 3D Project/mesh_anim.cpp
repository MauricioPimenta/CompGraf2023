...
int movKick = -1;
int movDance = -1;
// Added by Maurício Bittencourt Pimenta
int movWalk = -1;
...
void init ()
{
    ...
    movIdle = lutador.loadMeshAnim("Animations/idle/ninja_idle_######.obj", 40);
    movPunch = lutador.loadMeshAnim("Animations/punch/ninja_punching_######.obj", 35);
    movKick = lutador.loadMeshAnim("Animations/kick/ninja_kick_######.obj", 35);
    movDance = lutador.loadMeshAnim("Animations/dance/ninja_dance_######.obj", 90);
	movWalk = lutador.loadMeshAnim("Animations/walking/walking######.obj", 43);
    lutador.loadTexture("Textures/Ninja/Ch24_1001_Diffuse.bmp");
...
void desenhaJogador(){
    ...
    glPushMatrix();
		...
		// Added by Maurício Bittencourt Pimenta
		else if (walking && !acting){
			lutador.drawInit(movWalk);
			acting = 1;
		}
...
{
    switch(key){
		case 'w':	// Added by Maurício Bittencourt Pimenta
			if (acting) break;
			walking = 1;
			break;
		case 'a':
